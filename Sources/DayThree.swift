...
class DayThree {
    ...
    let validPartSymbols =  ["*", "%", "@", "#", "=", "/", "&", "+", "$", "-"]
    
    let gearSymbol = "*"
    
    init() {
        self.lines = self.input.components(separatedBy: .newlines)
    }
    ...
    func calculateSumOfPartNumbers() -> Int {
        ...
        for index in 0..<self.lines.count {
            ...
            let numberRanges = line.ranges(of: OneOrMore(.digit))
            
            
            
            for numberRange in numberRanges {
                guard let number = Int(line[numberRange]) else {
                    continue
                }
                
                let extendedRange = Self.extendRange(numberRange, line: line)
                
                if let previousLine = lines[safe: index-1] {
                    let ceiling = String(previousLine[extendedRange])
                    if validPartSymbols.contains(where: { ceiling.contains($0) }) {
                        validPartNumbers.append(number)
                        continue
                    }
                }
                
                if validPartSymbols.contains(where: { line[extendedRange].contains($0) }) {
                    validPartNumbers.append(number)
                    continue
                }
                
                if let nextLine = lines[safe: index+1] {
                    let floor = String(nextLine[extendedRange])
                    if validPartSymbols.contains(where: { floor.contains($0) }) {
                        validPartNumbers.append(number)
                        continue
                    }
                }
    ...
    func calculateSumOfGears() -> Int {
        ...
        for index in 0..<self.lines.count {
            let gearsInPreviousLine = numbersPerGear.keys.filter({ $0.lineIndex == index-1 })
            let line = self.lines[index]
            let gearsInLine = numbersPerGear.keys.filter({ $0.lineIndex == index })
            let gearsInNextLine = numbersPerGear.keys.filter({ $0.lineIndex == index+1 })
            let numberRanges = line.ranges(of: OneOrMore(.digit))
            for numberRange in numberRanges {
                guard let number = Int(line[numberRange]) else {
                    continue
                ...
                let extendedRange = Self.extendRange(numberRange, line: line)
                let adjacentGears = (gearsInPreviousLine + gearsInLine + gearsInNextLine).filter({ gear in
                    extendedRange.contains(gear.index)
                })
                for gear in adjacentGears {
                    numbersPerGear[gear]?.append(number)
                }
            }
        }
        let validGearRatios = numbersPerGear.filter({ $0.value.count == 2 }).values.map({ $0[0] * $0[1] })
        return validGearRatios.reduce(0, +)
    }
    
    static func extendRange(_ range: Range<String.Index>, line: String) -> Range<String.Index> {
        var lowerBound = range.lowerBound
        var upperBound = range.upperBound
        if upperBound != line.endIndex {
            upperBound = line.index(after: upperBound)
        }
        if lowerBound != line.startIndex {
            lowerBound = line.index(before: lowerBound)
        }
