...
extension DayTwo {
    ...
    struct Game {
        ...
        let minimumCubes: (red: Int, green: Int, blue: Int)
        
        init?(string: String) {
            let gameIDRef = Reference(Int.self)
            let setsRef = Reference(Substring.self)
            ...
            guard let output = try? regex.wholeMatch(in: string) else {
                return nil
            }
            self.id = output[gameIDRef]
            let shownSets = output[setsRef].components(separatedBy: "; ").map({ Set(string: $0) })
            self.shownSets = shownSets
            
            var red: Int = 0
            var green: Int = 0
            var blue: Int = 0
            for set in shownSets {
                if set.red > 0 && set.red > red {
                    red = set.red
                }
                if set.green > 0 && set.green > green {
                    green = set.green
                }
                if set.blue > 0 && set.blue > blue {
                    blue = set.blue
                }
            }
            self.minimumCubes = (red, green, blue)
        }
        
        func isPossibleWith(red: Int, green: Int, blue: Int) -> Bool {
            for set in shownSets {
                if set.red > red {
                    return false
                }
...
extension DayTwo.Game {
 
    struct Set {
        
        /// The regex for getting the red value of the set.
        /// Is a seperate regex because there are bugs with Lookahead/Lookbehind in Swift.
        private static let redRegex = Regex {
            TryCapture {
                OneOrMore(.digit)
            } transform: {
                Int(String($0))
        ...
        /// The regex for getting the green value of the set.
        /// Is a seperate regex because there are bugs with Lookahead/Lookbehind in Swift.
        private static let greenRegex = Regex {
            TryCapture {
                OneOrMore(.digit)
            } transform: {
                Int(String($0))
        ...
        /// The regex for getting the blue value of the set.
        /// Is a seperate regex because there are bugs with Lookahead/Lookbehind in Swift.
        private static let blueRegex = Regex {
            TryCapture {
                OneOrMore(.digit)
            } transform: {
                Int(String($0))
        ...
        init(string: String) {
            let red   = (try? Self.redRegex.firstMatch(in: string)?.output.1) ?? 0
            let green = (try? Self.greenRegex.firstMatch(in: string)?.output.1) ?? 0
            let blue  = (try? Self.blueRegex.firstMatch(in: string)?.output.1) ?? 0
            self.red = red
            self.green = green
            self.blue = blue
        }
...
class DayTwo {
    ...
    init() {
        let lines = self.input.components(separatedBy: .newlines)
        self.games = lines.compactMap({ Game(string: $0) })
    }
    
    
    func calculateSumOfIDs() -> Int {
        let possibleGames = self.games.filter({ $0.isPossibleWith(red: 12, green: 13, blue: 14) })
        let sumOfIDs = possibleGames.reduce(0, { $0 + $1.id })
        return sumOfIDs
    }
    
    func calculateSumOfGamePowers() -> Int {
        let powers = self.games.map({ $0.minimumCubes }).map({ $0 * $1 * $2 })
